function [ops, res] = svd_cluster(ops, Ly, Lx, U, Sv)
U = reshape(U, Ly, Lx,[]);
...
S = normc(S); % nPixel X spatial basis function

%%
StS = S' * S;
StU = S' * U'; % nBasis^2 X nSVD
...
for k = 1:niter
    for i = 1:Nk
        ...
        if numel(ix)==0
            ...
            LtU(i,:) = M(ix) * U(:, ix)'; % coefficent X latent variables to each cluster Nk x nSVD
            LtL(i,i) = sum(M(ix).^2); % diagonal matrix of coefficients Nk * Nk
            LtS(i,:) = M(ix) * S(ix, :); % spatial smoothing Nk * nBasis^2
        end
    end
    %
    covL = [LtL LtS; LtS' StS]; % Nk+nBasis^2 X Nk+nBasis^2
    LtXS = [LtU; StU]; % (Nk + nBasis^2) X nSVD
    Lam = (covL + 1e-4 * Ireg) \ LtXS;
    vs = normc(Lam(1:Nk, :)');
    neuropil = Lam(Nk + [1:nBasis^2], :)' * S';
    xs = vs' * (U - neuropil); % nSVD x nPixel
    [M, iclust] = max(xs,[],1); % allocate pixels to classes by finding maximal SVD dimension of the pixel
    if 1
        %---------------------------------------------%
        % exclude allocate the dimension of each pixel by assign them zeros
        xs(iclust + (0:Nk:numel(xs)-1)) = 0; 
        % reallocate nonzero dimensions 
        ...
        for j = 1:Nk
            icl{j} = iclust==j;
            if ~isempty(icl{j})
                dMk(j) = sum(dM(icl{j})); % deviation of each cluster 
            end
        end
        ...
        vlk(~ison) = Inf;
        while indrem>0
            % check convergence of each cluster, find the cluster which
            % has largest deviation between the initial allocation and the
            % reallocation. Reallocat this cluster
